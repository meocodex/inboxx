...
import { contatos, contatosConexoes } from './contatos.js';
...
export const conversas = pgTable('conversas', {
  ...
  // conexaoId é nullable - permite excluir conexão sem perder conversa
  conexaoId: uuid('conexao_id').references(() => conexoes.id, { onDelete: 'set null' }),
  // contatoConexaoId referencia a tabela pivot (preserva histórico)
  contatoConexaoId: uuid('contato_conexao_id').references(() => contatosConexoes.id, { onDelete: 'set null' }),
  ...
  index('conversas_contato_conexao_id_idx').on(t.contatoConexaoId),
]);
...
export const conversasRelations = relations(conversas, ({ one, many }) => ({
  ...
  contatoConexao: one(contatosConexoes, { fields: [conversas.contatoConexaoId], references: [contatosConexoes.id] }),
